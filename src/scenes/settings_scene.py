...
class SettingsScene(Scene):
    ...
    def handle_event(self, event):
        ...
        if event.key == pygame.K_LEFT and focused in ("rate", "volume", "marks"):
            self._adjust(focused, -1)
            return
        if event.key == pygame.K_RIGHT and focused in ("rate", "volume", "marks"):
            self._adjust(focused, 1)
            return
        ...
        elif result in ("rate", "volume", "marks"):
